<script lang="ts">
	...
	type SavedActions = { followupSent: boolean; linkedinOpened: boolean };
	type NextCta = 'followup' | 'linkedin' | 'scan' | null;

	const SESSION_KEY = 's2c.review';
	...
	let savedActions = $state<SavedActions>({ followupSent: false, linkedinOpened: false });
	...
	function buildPersistPayload(): string {
		// Deep stringify subscribes the calling $effect to nested $state mutations.
	...
	function restoreReview() {
		...
		try {
			...
			const ts = typeof data.saveTriggeredAt === 'number' ? data.saveTriggeredAt : null;
			...
			contact = parsed.data;
			reviewStage = data.reviewStage === 'saved' ? 'saved' : 'edit';
	...
	function triggerReturnNudge() {
		...
		if (step !== 'review' || reviewStage !== 'saved') return;
	...
	$effect(() => {
		...
		const currentStage = reviewStage;
		if (!storageReady || typeof window === 'undefined') return;
		if (currentStep !== 'review' || currentStage !== 'saved') {
			clearStoredReview();
			return;
		}
		try {
